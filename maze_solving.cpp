...
void Maze_Solving::ucs( const int &first_y, const int &first_x, const int& types ) {
    ...
    enum class Types : int {
        ...
        Interval = 2    //Cost Function 以區間來計算，每一個區間 Cost 差1000，距離終點越遠 Cost 越大
    };
...
void Maze_Solving::a_star(const int &first_y, const int &first_x){
    struct Node {
        int __Cost;    //Cost Function 以區間計算，每個區間 Cost 差1000
    ...
    std::priority_queue<Node, std::deque<Node>, std::greater<Node>> result;    //待走的結點，greater代表小的會在前面，由小排到大
    constexpr int interval_y = MAZE_HEIGHT / 10, interval_x = MAZE_WIDTH / 10;    //分 10 個區間
    int cost = (static_cast<int>(first_y/interval_y) < static_cast<int>(first_x/interval_x)) ? (10 - static_cast<int>(first_y/interval_y))*10 : (10 - static_cast<int>(first_x/interval_x))*10;    //Cost 以區間計算，兩個相除是看它在第幾個區間，然後用總區間數減掉，代表它的基礎 Cost，再乘以1000
    int weight = cost + pow_two_norm(first_y, first_x);   //權重以區間(Cost) + Two_Norm 計算

...
Making_Maze::Making_Maze(Animator* A) : Maze(A) {
}
...
void Making_Maze::random_prim_make_maze( const int &first_y, const int &first_x ) {
    ...
    std::deque<std::pair<int, int>> confirm_wall;
    while ( !wall.empty() ) {
        ...
        if ( maze[temp_y][temp_x] == 1 ) {    //如果抽到的那格確定是牆再去判斷，有時候會有一個牆重複被加到清單裡的情形
            ...
            if ( ( up_node == 2 && down_node == 2 ) || ( left_node == 2 && right_node == 2 ) ) {
                confirm_wall.emplace_back(wall.at(random_index));
    ...
    int confirm_wall_num = confirm_wall.size()/2;
    for(int i = rand()%(confirm_wall_num / 4); i != 0; --i) {
        random_index = rand() % confirm_wall.size();
        maze[confirm_wall.at(random_index).first][confirm_wall.at(random_index).second] = 0;
        confirm_wall.erase(confirm_wall.begin() + random_index);
        animator->update( 50 );
    }

...
[[maybe_unused]] constexpr int BEGIN_X = 0;
[[maybe_unused]] constexpr int BEGIN_Y = 1;
constexpr int END_Y = MAZE_HEIGHT - 2;
constexpr int END_X = MAZE_WIDTH - 2;
...
int main(int argc, char *argv[]) {
    ...
    QObject::connect(runButton, &QPushButton::clicked, [&box, &w]()
    {
        ...
        else if(box.currentText() == "UCS") w.ucs(1,0);
        else if(box.currentText() == "DFS") w.dfs({1, 0});
        w.repaint();
    });
    ...
    //    }
    //}
    const std::pair<int, int> directions[4]{ { 1, 0 }, { -1, 0 }, { 0, -1 }, { 0, 1 } };    //上、下、右、左
    ...
    while ( !wall.empty() ) {
        ...
        if(maze[temp_y][temp_x] == 1) {    //如果抽到的那格確定是牆再去判斷，有時候會有一個牆重複被加到清單裡的情形
            ...
            if( if_in_maze(temp_y, temp_x, -1, 0) ) up_node = maze[temp_y - 1][temp_x];
            if( if_in_maze(temp_y, temp_x, 1, 0) ) down_node = maze[temp_y + 1][temp_x];
            if( if_in_maze(temp_y, temp_x, 0, -1) ) left_node = maze[temp_y][temp_x - 1];
            if( if_in_maze(temp_y, temp_x, 0, 1) ) right_node = maze[temp_y][temp_x + 1];
            ...
            if ( ( up_node == 2 && down_node == 2 ) || ( left_node == 2 && right_node == 2 ) )
                ...
                if ( up_node == 2 && down_node == 0 ) {
                    std::cout << "now draw " << temp_y + 1 << " " << temp_x << " to 2" << '\n';
                    ...
                    std::cout << "now draw " << temp_y << " " << temp_x + 1 << " to 2" << '\n';
